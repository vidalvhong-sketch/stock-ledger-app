...
router.post('/', requirePermission('manage_schedule'), (req, res) => {
...
router.put('/:id', requirePermission('manage_schedule'), (req, res) => {
...
router.delete('/:id', requirePermission('manage_schedule'), (req, res) => {
  const shift = db.prepare('SELECT * FROM shifts WHERE id = ?').get(req.params.id);
  if (!shift) return res.status(404).json({ error: 'Shift not found' });
