...
// Team Lead can always approve discount/refund/void requests — that's the
// entire point of the role. Manager needs the specific permission granted.
// Admin can always approve everything.
function canApprove(db, userId) {
  if (!userId) return false;
  const row = db.prepare('SELECT role, manager_permissions FROM users WHERE id = ? AND active = 1').get(userId);
  if (!row) return false;
  if (row.role === 'admin' || row.role === 'team_lead') return true;
  if (row.role !== 'manager') return false;
  return !!parsePermissions(row.manager_permissions).approve_discounts_refunds_voids;
}

module.exports = { PERMISSIONS, PERMISSION_KEYS, parsePermissions, hasPermission, requirePermission, canApprove };
