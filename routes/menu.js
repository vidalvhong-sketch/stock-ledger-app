...
router.post('/', adminRequired, (req, res) => {
  const { name, category, price, barcode, recipe } = req.body;
  ...
  logAudit({ userId: req.user.id, userName: req.user.name, action: 'create_menu_item', targetType: 'menu_item', targetId: id, details: { name: name.trim(), price: Number(price) } });
...
router.post('/:id/recipe-image', adminRequired, uploadRecipeImage.single('image'), (req, res) => {
  const item = db.prepare('SELECT * FROM menu_items WHERE id = ?').get(req.params.id);
  if (!item) return res.status(404).json({ error: 'Item not found' });
...
router.delete('/:id/recipe-image', adminRequired, (req, res) => {
  const item = db.prepare('SELECT * FROM menu_items WHERE id = ?').get(req.params.id);
  if (!item) return res.status(404).json({ error: 'Item not found' });
  if (item.recipe_image) {
