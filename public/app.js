...
function addToCart(itemId) {
  ...
  if (line) line.qty += 1;
  else state.cart.push({ itemId: item.id, name: item.name, price: item.price, qty: 1 });
  render();
}
...
function tabsForApp(mode) {
  if (mode === 'admin') {
    return [['overview', 'Overview', 'dashboard'], ['team', 'Team', 'team'], ['audit', 'Audit Log', 'audit'], ['settings', 'Settings', 'admin']];
  }
  if (mode === 'pos') {
  ...
  if (mode === 'time') {
    ...
    if (state.user.role === 'admin') tabs.push(['team', 'Team', 'team'], ['payroll', 'Payroll', 'payroll'], ['eval', 'Evaluate', 'reports']);
    return tabs;
  }
  const tabs = [
  ...
  if (state.user.role === 'admin') tabs.push(['admin', 'Admin', 'admin']);
  return tabs;
}
...
function renderShell() {
  ...
  if (state.user.role === 'admin') apps.push(['admin', 'Admin', 'admin']);
...
function renderTab() {
  ...
  if (state.appMode === 'time') {
    switch (state.tab) {
      ...
      case 'team': return state.user.role === 'admin' ? renderTimeTeam() : '<div>Not authorized.</div>';
      case 'payroll': return state.user.role === 'admin' ? renderTimePayroll() : '<div>Not authorized.</div>';
      case 'eval': return state.user.role === 'admin' ? renderTimeEval() : '<div>Not authorized.</div>';
      default: return '';
    }
  }
  switch (state.tab) {
    ...
    case 'admin': return state.user.role === 'admin' ? renderAdmin() : '<div>Not authorized.</div>';
    default: return '';
  }
}
  ...
  <div class="fade-in">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-bottom:20px;">
      ${filtered.map(item => {
        ...
        return `<div class="card pos-item-card" data-id="${item.id}" style="cursor:pointer;padding:14px;position:relative;${inCart ? 'border-color:var(--accent);' : ''}">
          ${inCart ? `<span class="badge badge-accent" style="position:absolute;top:10px;right:10px;">×${inCart.qty}</span>` : ''}
          <div style="font-weight:600;font-size:14px;">${escapeHtml(item.name)}</div>
          <div class="font-mono" style="color:var(--accent-soft);font-size:15px;font-weight:700;margin-top:6px;">${fmtMoney(item.price)}</div>
        </div>`;
      }).join('')}
    </div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;max-height:200px;overflow-y:auto;" class="scrollbar-thin">
        ${state.cart.map(l => `
          <div style="display:flex;justify-content:space-between;align-items:center;font-size:13.5px;">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <button class="btn btn-ghost cart-qty-btn" data-id="${l.itemId}" data-delta="-1" style="padding:2px 9px;font-size:14px;">−</button>
...
function attachShellHandlers() {
  ...
  document.getElementById('newUserBtn')?.addEventListener('click', () => {
    state.editUser = { id: null, name: '', role: 'staff', active: 1, _existing: false };
  ...
  document.querySelectorAll('.edit-user-btn').forEach(b => b.addEventListener('click', () => {
    ...
    state.editUser = Object.assign({}, u, { _existing: true });
    state.showHrDetails = false; state.staffDocuments = {};
    render();
  }));
  ...
  document.querySelectorAll('.pos-item-card').forEach(b => b.addEventListener('click', () => addToCart(b.getAttribute('data-id'))));
  document.querySelectorAll('.cart-qty-btn').forEach(b => b.addEventListener('click', () => changeCartQty(b.getAttribute('data-id'), Number(b.getAttribute('data-delta')))));
  ...
  document.getElementById('newMenuItemBtn')?.addEventListener('click', () => {
    state.editMenuItem = { id: null, name: '', category: '', price: '', barcode: '', _existing: false };
    render();
  });
  document.querySelectorAll('.edit-menu-item-btn').forEach(b => b.addEventListener('click', () => {
    ...
    state.editMenuItem = Object.assign({}, item, { _existing: true });
    render();
  }));
