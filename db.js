...
function initDb() {
  db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      name TEXT NOT NULL,
      role TEXT NOT NULL CHECK(role IN ('admin','manager','team_lead','staff')),
    ...
    -- Sizes/variants for a menu item (e.g. 16oz / 22oz), each with its own
    -- price. An item with rows here is sold by size, not its flat price.
  ...
  // The users table was created with a hardcoded CHECK(role IN ('admin','staff')).
  // SQLite can't alter a CHECK constraint in place, so widening it to allow
  // 'manager' and 'team_lead' requires rebuilding the table. This only runs
  // once — after it runs, the stored CREATE TABLE text no longer matches the
  // old constraint, so this check is false on every later boot.
  const usersTableSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='users'").get().sql;
  if (usersTableSql && usersTableSql.includes("CHECK(role IN ('admin','staff'))")) {
    // Many other tables (time_entries, sales, shifts, etc.) hold a live FK
    // reference to users(id) — dropping the table while foreign_keys is ON
    // fails outright. Turn it off just for this rebuild; the rename at the
    // end makes every existing FK resolve to the new table transparently,
    // since SQLite resolves FK targets by table name, not by object identity.
    ...
    db.exec(`
      CREATE TABLE users_new (
        ...
        email TEXT,
        position TEXT,
        address TEXT, emergency_contact TEXT, date_of_birth TEXT, tin TEXT, sss_number TEXT,
        philhealth_number TEXT, pagibig_number TEXT, contact_number TEXT, education TEXT,
        date_hired TEXT, termination_date TEXT, hr_notes TEXT,
        manager_permissions TEXT
      );
      INSERT INTO users_new (
        id, name, role, pin_hash, active, created_at, hourly_rate, email, position,
        address, emergency_contact, date_of_birth, tin, sss_number,
        philhealth_number, pagibig_number, contact_number, education,
        date_hired, termination_date, hr_notes
      )
    ...
    if (orphans.length > 0) {
      console.error('WARNING: foreign_key_check found issues after users table rebuild:', orphans);
    }
    db.pragma('foreign_keys = ON');
  }
  // Fresh installs create the table with the new constraint directly, but
  // won't have this column yet either way — same safe-add pattern as everywhere else.
  const userColsAfterRoleMigration = db.prepare("PRAGMA table_info(users)").all().map(c => c.name);
  if (!userColsAfterRoleMigration.includes('manager_permissions')) {
    db.exec('ALTER TABLE users ADD COLUMN manager_permissions TEXT');
  }
